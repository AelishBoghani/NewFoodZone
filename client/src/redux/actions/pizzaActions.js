...
export const filterPizzas=(searchKey,category)=> async dispatch=>{
    ...
    try{
        const response = await axios.get('/api/pizzas/getallpizzas')
        fliteredPizzas=response.data.filter(pizza=>pizza.name.toLowerCase().includes(searchKey))

        if(category!='all'){
            fliteredPizzas=response.data.filter(pizza=>pizza.category.toLowerCase()==category)
        }
        dispatch({type:'GET_PIZZAS_SUCCESS', payload : fliteredPizzas})
    }catch(error){ 
        dispatch({type:'GET_PIZZAS_FAILED',payload: error})
    }
}
